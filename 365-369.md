...
构建网络生成器部分,函数参数:gen_z:生成器输入张量;gen_out_dim:生层器的输出形状;num_hiddern_units:神经元的数量/隐藏层中的单位：reuse_vars:tf.variable_scope中的重用变量；leaky_relu_alpha:Leaky ReLU参数;函数返回:sigmoid_out;logits_layer

```tf.variable_scope('generator',reuse=reuse_vars)```
定义生成器隐藏层
```hidden_layer_1=tf.layers.dense(qen_z,num_hiddern_units,activation=None)```
将hidden_layer_1的输出送到leaky relu中
...
```tf.variable_scope('discriminator',reuse=reuse_vars)```
定义判别器隐藏层
